...
class Org {
    ...
    static readMapOfLocallyValidatedMetaConfig(userFilenames, concurrency = 3) {
        ...
        const fileDoesntExistFilter = Promise.all(_.map(userFilenames, (fileName) => {
            ...
            .filter((statsAndContent) => !_.isNil(statsAndContent[0]) && !_.isNil(statsAndContent[1]) && _.isNil(statsAndContent[1].scratchAdminUsername));
        return fileDoesntExistFilter.then((fileContentsAndMeta) => {
            ...
            // Init crypto before spawning lots getConfigs, all calling init at the same time.
            return new Crypto().init().then(() => Promise.map(fileContentsAndMeta, (fileContentAndMeta) => {
                /**
