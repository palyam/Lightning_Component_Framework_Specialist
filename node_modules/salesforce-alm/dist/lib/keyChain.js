...
const _isValidFileAccess = function (cb) {
    fs.stat(this.secretFile, (err, stats) => {
        if (!_.isNil(err)) {
            cb(err);
        }
        else {
            ...
            if (octalModeStr === EXPECTED_OCTAL_PERM_VALUE) {
                ...
                cb(almError('genericUnixKeychainInvalidPerms', null, 'genericUnixKeychainInvalidPermsAction', [this.secretFile, EXPECTED_OCTAL_PERM_VALUE]));
            }
        }
    });
};
function _writeFile(opts, fn) {
    ...
    fs.writeJson(this.secretFile, obj, (writeError) => {
        if (writeError) {
            fn(writeError);
        }
        else {
...
class GenericUnixKeychainAccess {
    constructor() {
        this.secretFile = path.join(srcDevUtil.getGlobalHiddenFolder(), 'key.json');
    }
    getPassword(opts, fn) {
        // validate the file in .sfdx
        _isValidFileAccess.call(this, (fileAccessError) => {
            // the file checks out.
            if (_.isNil(fileAccessError)) {
                // read it's contents
                fs.readJson(this.secretFile, (readJsonErr, readObj) => {
                    ...
                    if (_.isNil(readJsonErr)) {
                        ...
                        if ((opts.service === readObj.service) && (opts.account === readObj.account)) {
                            ...
                            fn(almError('genericUnixKeychainServiceAccountMismatch', [this.secretFile], 'genericUnixKeychainServiceAccountMismatchAction', null));
                        }
                    }
                    else {
                        fn(readJsonErr);
    ...
    setPassword(opts, fn) {
        ...
        _isValidFileAccess.call(this, fileAccessError => {
            ...
            if (!_.isNil(fileAccessError)) {
                ...
                if (fileAccessError.code === 'ENOENT') {
                    // create the file
                    fs.ensureFile(this.secretFile, ensureFileError => {
                        if (!ensureFileError) {
                            // set up the perms correctly
                            fs.chmod(this.secretFile, '600', chmodError => {
