...
class MdRetrieveReportApi {
    ...
    _checkStatus(options) {
        const org = this.org;
        return new CheckStatus(options.wait, constants.DEFAULT_MDAPI_POLL_INTERVAL_MILLISECONDS, this._print.bind(this, options), this.force.mdapiCheckRetrieveStatus.bind(org.force, org, options.jobid)).handleStatus();
    }
    ...
    _handleResult(options, result) {
        ...
        return this.handleZipResult(options, result, this)
            .then(() => result);
    }
    ...
    _print(options, result) {
        if (!this.isJsonOutput) {
            ...
            if (result.done && options.verbose) {
                ...
                const files = hasFiles ? _.chain(result.fileProperties)
                    .sortBy([function (o) { return o.fullName.toUpperCase(); }])
                    .sortBy([function (o) { return o.fileName.toUpperCase(); }])
                    .sortBy([function (o) { return o.type.toUpperCase(); }])
                    .value() :
                    [];
                this._log('');
            ...
            if (result.timedOut) {
                ...
                this._log(messages().getMessage('mdRetrieveCommandCliWaitTimeExceededError', options.wait));
            }
            this._log('');
        }
        return result;
    ...
    validate(context) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let stashedValues = yield Stash.list(Stash.Commands.MDAPI_RETRIEVE);
            if (!options.jobid) {
