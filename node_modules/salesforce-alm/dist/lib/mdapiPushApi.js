...
class MdapiPushApi {
    ...
    _checkForConflicts(options) {
        if (options.forceoverwrite) {
            return this.swa.getAggregateSourceElements(CHANGES_ONLY);
        }
        else {
            const statusApi = new SrcStatusApi(this.scratchOrg);
            return statusApi.doStatus({ local: true, remote: true })
                ...
                if (conflicts.length > 0) {
                    ...
                    return this.swa.getAggregateSourceElements(CHANGES_ONLY); // if no conflicts return origin elements to push
                }
            });
        }
    }
